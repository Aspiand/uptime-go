...
func NotifyIncident(incident *models.Incident, severity incident.Severity, attributes map[string]any) (uint64, error) {
	...
	if err != nil {
		log.Error().Err(err).Msgf("[webhook] Failed to send incident notification for %s: failed to get server ip address", incident.Monitor.URL)
		return 0, err
	}
	...
	if err != nil {
		log.Error().Err(err).Msgf("[webhook] Failed to send incident notification for %s", incident.Monitor.URL)
		return 0, err
	}

	if response.StatusCode != http.StatusCreated {
		err := fmt.Errorf("failed to create incident, received status code %d. Body: %s", response.StatusCode, string(body))
		log.Error().Err(err).Msg("[webhook]")
		return 0, err
	}
	...
	log.Info().Msgf("[webhook] Successfully created incident for monitor %s - Reason: %s - Incident Master ID: %d", incident.Monitor.URL, incident.Type, result.Data.ID)
	return result.Data.ID, nil
}

func UpdateIncidentStatus(incident *models.Incident, status incident.Status) error {
	if incident.IncidentID == 0 {
		log.Error().Msgf("[webhook] Failed to update incident status for %s: incident_id not set", incident.ID)
	}
	...
	if err != nil {
		log.Error().Err(err).Msgf("[webhook] Failed to send status update for incident %d", incident.IncidentID)
		return err
	}
	...
	log.Info().Msgf("[webhook] Successfully updated status for incident %d to '%s'. Message: %s", incident.IncidentID, status, result.Message)
	return nil
}
